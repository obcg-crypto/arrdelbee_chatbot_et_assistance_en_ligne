...
import { INFO_MANAGE_PATH ,CHATBOT_PATH} from '@/constants/tabsLinks'
    ...
    const MENU_LIST: Array<MenuItemType> = [
        {
            ...
            label: "Agent ArrdelBee",
            href: INFO_MANAGE_PATH,
            icon: <MdMonitorHeart size={20}/>
        },{
            label: "ChatBot",
            href: CHATBOT_PATH,
            icon: <PiWechatLogoBold size={20}/>
        },
    ]
                    ...
                    <div className="w-full h-full mb-2 flex flex-col justify-start items-center gap-2 border-b border-grayToneStroke">
                        {
                            MENU_LIST.map((item:MenuItemType, index:number)=>{
                                const ISACTIVE = decodeURI(PATHNAME).replaceAll("-"," ").trim().toLowerCase().includes(item.label.trim().toLowerCase()) || (PATH_ARRAY[PATH_ARRAY.length-1] === "dashboard" && item.label==="Accueil")
