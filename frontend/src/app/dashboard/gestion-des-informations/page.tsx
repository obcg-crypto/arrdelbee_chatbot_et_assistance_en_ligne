...
export default function MonitoringDefault(){
    ...
    const [CONVESATION_LIST2, setDiscussionList2] = useState<ConversationCardType[]>([]);

   

    useEffect(() => {
      const handleClientMessage = (msg: string) => {
        if (msg !== "") {
          console.log(msg);
          setDiscussionList2((prevList) => [
            ...prevList,
      ...
      // Attacher le gestionnaire d'événements pour les messages des clients
  ...
  const handleSubmit = async () => {
    ...
    socket.emit("bot message", userInput2);
    ...
    try {
      ...
      const response = await fetch('http://localhost:2000/api/message/post', {
        ...
        body: JSON.stringify({
            content: userInput2,
            sender: '65a7d8db19a88c910d4bc7c2',
            receiver: '65a7d86019a88c910d4bc7c0',
            date: currentTime
        }),
      ...
      setDiscussionList2((prevList) => [
        ...prevList,
        { texte: userInput2, time: currentTime, type: "client" },
      ]);
        ...
        <div>
            ...
            {/* Chatbot Page */}
             <div className=" relative w-full bg-white">
                    ...
                    {
                        CONVESATION_LIST2.map((conversation:ConversationCardType, index:number)=>{
